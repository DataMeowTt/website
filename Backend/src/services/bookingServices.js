...
export const bookedBookingInDB = async ({
  ...
  try {
    ...
    // =================================================================
    // SỬA LỖI Ở ĐÂY: Truyền `objectUserId`, `objectCenterId` VÀ `session`
    // =================================================================

    // Truyền vào `objectUserId` (ObjectId) và `session`
    const completedCount = await updateCompletedBookingsForUser(objectUserId, session);
    console.log(`User ${userId} có ${completedCount} booking đã thanh toán, hãy chờ để Admin duyệt.`);

    // Truyền vào `objectUserId` (ObjectId) và `session`
    await updateChartForCompleted(objectUserId, booking.date, session);
    console.log(`Đã cập nhật chart data cho user ${userId} với completed +1`);

    // Truyền vào `objectUserId` và `objectCenterId` (ObjectId) và `session`
    await updateFavouriteCenter(objectUserId, objectCenterId, session);
